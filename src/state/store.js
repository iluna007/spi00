import { DEFAULT_NODE_COLORS } from '../components/graphConstants'
import { NODE_TYPE_LABELS_BASE } from '../components/graphConstants'

const defaultVisibleTypes = Object.keys(NODE_TYPE_LABELS_BASE).reduce(
  (acc, t) => ({ ...acc, [t]: true }),
  {}
)
...
function graphReducer(state, action) {
  switch (action.type) {
    ...
    case 'GRAPH/SET_DATA_READY':
      return { ...state, dataReady: !!action.payload }
    case 'GRAPH/SET_NODE_REL_SIZE':
      return { ...state, nodeRelSize: action.payload }
    case 'GRAPH/SET_LINK_WIDTH_SCALE':
      return { ...state, linkWidthScale: action.payload }
    case 'GRAPH/SET_LINK_DISTANCE':
      return { ...state, linkDistance: action.payload }
    case 'GRAPH/SET_CHARGE_STRENGTH':
      return { ...state, chargeStrength: action.payload }
    case 'GRAPH/SET_SHOW_NODE_LABELS':
      return { ...state, showNodeLabels: !!action.payload }
    case 'GRAPH/SET_LABEL_SCALE':
      return { ...state, labelScale: action.payload }
    case 'GRAPH/SET_LABEL_TEXT_COLOR':
      return { ...state, labelTextColor: action.payload }
    ...
    case 'GRAPH/SET_NODE_COLORS':
      return {
        ...state,
        nodeColors: { ...state.nodeColors, ...action.payload },
      }
    case 'GRAPH/SET_VISIBLE_TYPES':
      return { ...state, visibleTypes: action.payload }
    case 'GRAPH/SET_SEARCH_QUERY':
      return { ...state, searchQuery: action.payload }
    case 'GRAPH/SET_SEARCH_TYPE_FILTER':
      return { ...state, searchTypeFilter: action.payload }
    case 'GRAPH/SET_SEARCH_TYPE_OPEN':
      return { ...state, searchTypeOpen: !!action.payload }
    default:
      return state
  }
}
...
function selectionReducer(state, action) {
  switch (action.type) {
    ...
    case 'SELECTION/LOAD_MD_START':
      return { ...state, mdLoading: true }
    case 'SELECTION/LOAD_MD_SUCCESS':
      return { ...state, mdLoading: false, mdContent: action.payload }
...
function uiReducer(state, action) {
  switch (action.type) {
    ...
    case 'UI/SET_GRAPH_BG_COLOR':
      return { ...state, graphBgColor: action.payload }
    default:
      return state
  }
}
...
function context7Reducer(state, action) {
  switch (action.type) {
    case 'CONTEXT7/LOAD_HINTS_START':
      return { ...state, loadingNodeId: action.payload, error: null }
